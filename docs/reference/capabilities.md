...
`ResearchCapability` — the functional capabilities a provider exposes. The first
eight values are the historical `ModelCapability` set (kept as a backwards-
compatible alias); the rest were added to describe co-scientist behavior.
