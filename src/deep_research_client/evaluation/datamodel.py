...
class CellResult(ConfiguredBaseModel):
    ...
    cached: Optional[bool] = Field(default=None, description="""Whether the response was replayed from the client's cache rather than produced by calling the provider in this run. Recorded because a replayed cell is otherwise indistinguishable from a live one: the cache re-stamps start and end times for the current run, so duration does not give it away. A score computed over replayed cells may describe a model version that is months old, and two arms sharing a provider, model and parameters return one sample reported as two.""", json_schema_extra = { "linkml_meta": {'domain_of': ['CellResult']} })
