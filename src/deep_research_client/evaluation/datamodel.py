...
class RunManifest(ConfiguredBaseModel):
    ...
    cache_dir: Optional[str] = Field(default=None, description="""The response cache directory the run used, when it is not the client's default. A run whose replays came from a per-project cache has a different provenance from one that used the shared default, and `cache_enabled` alone does not say which.""", json_schema_extra = { "linkml_meta": {'domain_of': ['RunManifest']} })
